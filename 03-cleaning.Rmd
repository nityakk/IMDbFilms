...
```{r}
expDirecs <- byDirectors %>%
  group_by(directors) %>%
  count()
expDirecs
length(expDirecs$directors)

expDirecs2 <- expDirecs[which(expDirecs$n >= 20), ]
expDirecs2 <- unique(expDirecs2$directors)
length(expDirecs2)
```
...
```{r}
# 2(A)
byDirectors1 <- subset(byDirectors, directors %in% expDirecs2)
byDirectors1 <- byDirectors1[which(strtoi(byDirectors1$startYear) >= '2010'), ]

groupedDirec <- byDirectors4 %>%
...
cleveland <- groupedDirec_complete %>% 
  ungroup() %>%
...
# 2(B)
byDirectors2 <- byDirectors1[grepl("Drama", byDirectors3[["genres"]]), ]# | grepl("Comedy", byDirectors2[["genres"]]), ]

groupedDirec2 <- byDirectors2 %>%
  group_by(directors, titleType, genres, numVotes) %>%
  summarize(weightedRating = sum(averageRating * numVotes) / sum(numVotes))
groupedDirec_complete2 <- na.omit(groupedDirec)
...
df1 <- byDirectors[!is.na(basics_ratings_crew$writers),]
...
# for(director in groupedDirec$fileName) {

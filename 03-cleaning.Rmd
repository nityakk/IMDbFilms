...
```{r}
# foreach::director
# director <-  "Adam Feinsilver"
# dRSub <- directorRatings[which(directorRatings$primaryName == director), ]
# dRSub

for(director in directorRatings$primaryName) {
  dRSub <- directorRatings[which(directorRatings$primaryName == director), ]
  filename = paste("sources_short/directors/", director, "_Ratings.csv")
  write.csv(directorRatings, filename, row.names = FALSE)
}

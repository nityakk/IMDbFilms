...
-cleveland dot plot
```{r Cleveland Plot, fig.height=20, fig.width=10}
...
```{r Parcoord}
library(parcoords)
#groupedDirec_complete[sample(1:nrow(groupedDirec_complete),1000), c(2:5)]
parcoords(
...
```{r}
library(ggridges)
plot5 <- ggplot(df2, aes(x = averageRating, y = director_writer_pair, fill = titleType)) + geom_density_ridges()
```
...
```{r}
plot6 <- ggplot(new_filter, aes(x = reorder(director_writer_pair, -totalVotes), y = totalVotes, fill = titleType)) + geom_bar(stat = 'identity') + theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1)) + scale_x_discrete(label=abbreviate)
```
